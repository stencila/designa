...
const zoom$1 = require('./zoom-65ed5394.js');
...
function drag() {
  var filter = defaultFilter$1,
      ...
      gestures = {},
      listeners = zoom$1.dispatch("start", "drag", "end"),
      active = 0,
      mousedownx,
      mousedowny,
      mousemoving,
...
function forceCollide(radius) {
  ...
  function force() {
    ...
    for (var k = 0; k < iterations; ++k) {
      tree = zoom$1.quadtree(nodes, x$2, y$2).visitAfter(prepare);
      for (i = 0; i < n; ++i) {
        node = nodes[i];
...
function forceSimulation(nodes) {
  var simulation,
      ...
      velocityDecay = 0.6,
      forces = new Map(),
      stepper = zoom$1.timer(step),
      event = zoom$1.dispatch("tick", "end"),
      random = lcg();
...
function forceManyBody() {
  ...
  function force(_) {
    var i, n = nodes.length, tree = zoom$1.quadtree(nodes, x$1, y$1).visitAfter(accumulate);
...
function line(x$1, y$1) {
  ...
  function line(data) {
    ...
    if (context == null) output = curve(buffer = zoom$1.path());
...
function symbol(type, size) {
  ...
  function symbol() {
    var buffer;
    if (!context) context = buffer = zoom$1.path();
...
const graphEdgeToLinks = (graph) => graph.edges.reduce((links, edge) => {
...
const graphToGroupedGraph = (graph) => {
...
const isInterGroupLink = (source, target) => {
...
var emptyOn = zoom$1.dispatch("start", "end", "cancel", "interrupt");
var emptyTween = [];
...
function create(node, id, self) {
  ...
  schedules[id] = self;
  self.timer = zoom$1.timer(schedule, 0, self.time);
...
function interpolate(a, b) {
  var c;
  return (typeof b === "number" ? zoom$1.interpolateNumber
      : b instanceof zoom$1.color ? zoom$1.interpolateRgb
      : (c = zoom$1.color(b)) ? (b = c, zoom$1.interpolateRgb)
      : zoom$1.interpolateString)(a, b);
}
...
function transition_attr(name, value) {
  var fullname = namespace(name), i = fullname === "transform" ? zoom$1.interpolateTransformSvg : interpolate;
...
function transition_style(name, value, priority) {
  var i = (name += "") === "transform" ? zoom$1.interpolateTransformCss : interpolate;
...
function selection_transition(name) {
  ...
  if (name instanceof Transition) {
    ...
    id = newId(), (timing = defaultTiming).time = zoom$1.now(), name = name == null ? null : name + "";
  }
...
function zoom() {
  var filter = defaultFilter,
      ...
      duration = 250,
      interpolate = zoom$1.interpolateZoom,
      listeners = zoom$1.dispatch("start", "zoom", "end"),
      touchstarting,
      touchfirst,
      touchending,
      touchDelay = 500,
...
const getResourceSymbol = (resource, size = 512) => {
  switch (resource.type) {
    case 'Node': {
...
const ProjectGraph = class {
  ...
  render() {
    return (index$1.h(index$1.Host, null, index$1.h("div", { ref: (el) => (this.el = el), class: "projectGraph" })));
  }
...
ProjectGraph.style = {
  default: defaultProjectGraphCss,
  material: projectGraphMaterialCss
};

exports.stencila_project_graph = ProjectGraph;
