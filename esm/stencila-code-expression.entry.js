import { h, r as registerInstance, H as Host, a as getElement } from './index-364020fb.js';
import { z as ze, K as Ke, q as qe, c } from './schema-7962e868.js';
...
const renderNode = (node) => {
  ...
  else if (Ke('CodeTypes', node)) {
    return (h("pre", null,
      h("output", null, node.text)));
  }
  else if (qe('ImageObject', node)) {
    return h("stencila-image-object", { image: node });
  }
  else if (qe('Datatable', node)) {
...
const CodeExpressionComponent = class {
  ...
  onUpdateCodeChunk({ detail }) {
    if (detail.nodeId === this.el.id && qe('CodeExpression', detail.value)) {
