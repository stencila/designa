...
const y = require('./y-53b4bf2d.js');

const tan30 = y.sqrt(1 / 3);
const tan30_2 = tan30 * 2;

const symbolDiamond = {
  draw(context, size) {
    const y$1 = y.sqrt(size / tan30_2);
    const x = y$1 * tan30;
...
const kr = y.sin(y.pi / 10) / y.sin(7 * y.pi / 10);
const kx = y.sin(y.tau / 10) * kr;
const ky = -y.cos(y.tau / 10) * kr;

const symbolStar = {
  draw(context, size) {
    const r = y.sqrt(size * ka);
    const x = kx * r;
    const y$1 = ky * r;
    context.moveTo(0, -r);
    ...
    for (let i = 1; i < 5; ++i) {
      ...
      context.lineTo(c * x - s * y$1, s * x + c * y$1);
    }
    context.closePath();
  }
};

const sqrt3 = y.sqrt(3);

const symbolTriangle = {
  draw(context, size) {
    const y$1 = -y.sqrt(size / (sqrt3 * 3));
...
function drag() {
  var filter = defaultFilter$1,
      ...
      gestures = {},
      listeners = y.dispatch("start", "drag", "end"),
      active = 0,
      mousedownx,
      mousedowny,
      mousemoving,
...
var emptyOn = y.dispatch("start", "end", "cancel", "interrupt");
var emptyTween = [];
...
function create(node, id, self) {
  ...
  schedules[id] = self;
  self.timer = y.timer(schedule, 0, self.time);
...
function interpolate(a, b) {
  var c;
  return (typeof b === "number" ? y.interpolateNumber
      : b instanceof y.color ? y.interpolateRgb
      : (c = y.color(b)) ? (b = c, y.interpolateRgb)
      : y.interpolateString)(a, b);
}
...
function transition_attr(name, value) {
  var fullname = namespace(name), i = fullname === "transform" ? y.interpolateTransformSvg : interpolate;
...
function transition_style(name, value, priority) {
  var i = (name += "") === "transform" ? y.interpolateTransformCss : interpolate;
...
function selection_transition(name) {
  ...
  if (name instanceof Transition) {
    ...
    id = newId(), (timing = defaultTiming).time = y.now(), name = name == null ? null : name + "";
  }
...
function zoom() {
  var filter = defaultFilter,
      ...
      duration = 250,
      interpolate = y.interpolateZoom,
      listeners = y.dispatch("start", "zoom", "end"),
      touchstarting,
      touchfirst,
      touchending,
      touchDelay = 500,
...
const getResourceSymbol = (resource, size = 512) => {
  switch (resource.type) {
    case 'Node': {
      return y.Symbol(symbolTriangle, size);
    }
    case 'Symbol': {
      return y.Symbol(symbolDiamond, size);
    }
    case 'Source': {
      return y.Symbol(symbolStar, size);
    }
    case 'File': {
      return y.Symbol(symbolWye, size);
    }
    case 'Url': {
      return y.Symbol(symbolSquare, size);
    }
    case 'Module':
    default: {
      return y.Symbol(y.symbolCircle, size);
    }
  }
};
...
const ProjectGraph = class {
  constructor(hostRef) {
    ...
    this.drawGraph = (graph) => {
      ...
      const simulation = y.forceSimulation(groupedGraph.nodes)
        .force('link', y.forceLink(groupedGraph.links)
        ...
        : 30)
        .iterations(2))
        .force('link:groups', y.forceLink(groupedGraph.groupLinks)
        .strength((d) => (hasKind(d.source) && d.source.kind === 'Link') ||
        (hasKind(d.target) && d.target.kind === 'Link')
        ? 1
        : 0.375)
        .iterations(3))
        .force('charge', y.forceManyBody()
        .strength((d) => {
        return d.type === 'File' ? -100 : -1500;
      })
        .distanceMin(10)
        .distanceMax(1500))
        .force('collision', y.forceCollide(64).strength(1).iterations(4))
        .force('x', y.forceX().strength(0.02))
        .force('y', y.forceY().strength(0.02))
        .force('center', y.forceCenter())
        .velocityDecay(0.4)
        .alphaDecay(0.05);
      ...
      const drawHull = (fileNode) => {
        ...
        return hull ? (_a = y.line().curve(y.curveBasisClosed)(hull)) !== null && _a !== void 0 ? _a : '' : '';
      };
  ...
  render() {
    return (index.h(index.Host, null, index.h("div", { ref: (el) => (this.el = el), class: "projectGraph" })));
  }
...
//# sourceMappingURL=stencila-project-graph.cjs.entry.js.map