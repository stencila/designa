...
const CodeChunkComponent = class {
  constructor(hostRef) {
    ...
    this.editorLayoutChange = index.createEvent(this, "stencila-editor-layout-change", 7);
    /**
    ...
    this.handleLanguageChange = (e) => {
      if (this.programmingLanguage === undefined ||
        languageUtils.lookupFormat(this.programmingLanguage).name !== e.detail.name) {
  ...
  async getContents() {
    ...
    if (this.editorRef) {
      ...
      return schema.d({ text, programmingLanguage: language });
    }
  ...
  render() {
    return (index.h(index.Host, { class: {
      ...
      } }, index.h("figure", null, index.h("stencila-action-menu", null, index.h("stencila-menu", { menuPosition: "bottom-start", slot: "persistentActions" }, index.h(codeExecuteStatus.CodeExecuteStatus, { executeStatus: this.executeStatus, executeRequired: this.executeRequired, slot: "toggle" }), index.h("slot", { name: "code-dependencies" }), index.h("slot", { name: "code-dependents" })), this.isExecutable && (index.h("stencila-button", { icon: codeUtils.isPending(this.executeStatus) ? 'stop' : 'play', minimal: true, color: "key", class: "run", size: "xsmall", tooltip: codeUtils.isPending(this.executeStatus)
        ? 'Cancel'
        : this.shiftIsPressed
          ? 'Run only this code'
          : 'Run', iconOnly: true, slot: "persistentActions", onClick: (e) => this.execute(e.shiftKey ? 'Single' : 'Topological'), onMouseEnter: this.addKeyListeners, onMouseLeave: this.removeKeyListeners })), index.h("stencila-button", { minimal: true, color: "key", class: "sourceToggle", onClick: this.toggleCodeVisibility, icon: this.isCodeVisible ? 'eye-off' : 'eye', iconOnly: true, size: "xsmall", slot: "persistentActions", tooltip: `${this.isCodeVisible ? 'Hide' : 'Show'} Code\nShift click to set for all code` }), ")", this.isCodeVisible && (index.h("stencila-button", { minimal: true, color: "key", class: "layoutToggle", onClick: this.toggleEditorLayout, icon: this.isStacked ? 'layout-column' : 'layout-row', iconOnly: true, size: "xsmall", slot: "persistentActions", tooltip: `${this.isStacked ? 'Side by side' : 'Stacked'} view\nShift click to set for all code` }))), index.h("div", null, index.h("div", { class: {
        editorContainer: true,
        hidden: !this.isCodeVisible,
      } }, index.h("stencila-editor", { activeLanguage: this.programmingLanguage, executableLanguages: this.executableLanguages, autofocus: this.autofocus, executeHandler: () => this.onExecuteHandler(), keymap: this.keymap, readOnly: !this.isExecutable, "onStencila-language-change": this.handleLanguageChange, ref: (el) => {
        this.editorRef = el;
      } }, index.h("slot", { name: CodeChunkComponent.slots.text }), index.h("slot", { name: CodeChunkComponent.slots.errors }))), index.h("slot", { name: CodeChunkComponent.slots.outputs })), index.h("slot", { name: CodeChunkComponent.slots.label }), index.h("slot", { name: CodeChunkComponent.slots.caption }))));
  }
...
//# sourceMappingURL=stencila-code-chunk.cjs.entry.js.map