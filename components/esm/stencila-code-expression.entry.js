...
import { c } from './schema-3c9b003e.js';
...
const CodeExpressionComponent = class {
  constructor(hostRef) {
    ...
    this.generateContent = () => {
      var _a;
      return [
        h("span", { class: "actions" }, h("stencila-menu", { menuPosition: "bottom-start" }, h(CodeExecuteStatus, { executeStatus: this.executeStatus, executeRequired: this.executeRequired, slot: "toggle" }), h("slot", { name: "code-dependencies" }), h("slot", { name: "code-dependents" })), h("stencila-button", { "aria-label": "Run Code", class: "run", onClick: (e) => this.executeRef(e.shiftKey ? 'Single' : 'Topological'), color: "key", icon: isPending(this.executeStatus) ? 'stop' : 'play', iconOnly: true, minimal: true, size: "xsmall", tooltip: isPending(this.executeStatus)
            ? 'Cancel'
            : this.shiftIsPressed
              ? 'Run only this code'
              : 'Run', onMouseEnter: this.addKeyListeners, onMouseLeave: this.removeKeyListeners }), h("stencila-button", { "aria-label": `${this.isCodeVisible ? 'Hide' : 'Show'} Code`, class: "secondaryAction sourceToggle", onClick: this.toggleCodeVisibility, color: "key", icon: this.isCodeVisible ? 'eye-off' : 'eye', iconOnly: true, minimal: true, size: "xsmall", tooltip: `${this.isCodeVisible ? 'Hide' : 'Show'} Code\nShift click to set for all code` }), h("span", { class: "secondaryAction" }, h(LanguagePickerInline, { activeLanguage: (_a = this.programmingLanguage) !== null && _a !== void 0 ? _a : '', languageCapabilities: this.languageCapabilities, executableLanguages: this.executableLanguages, onSetLanguage: this.onSelectLanguage, disabled: this.readOnly })), h("span", { class: "text", contentEditable: !this.readOnly, spellcheck: "false", onBlur: this.removeHoverState, onInput: this.contentChangeHandler, tabIndex: this.isCodeVisible ? 0 : -1, role: "textbox" }, h("slot", { name: slots.text })), h("slot", { name: slots.errors })),
        h("svg", { class: "divider", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 8 24", preserveAspectRatio: "xMinYMin" }, h("path", { d: "M8 12L1 0H0v24h1l7-12z" })),
